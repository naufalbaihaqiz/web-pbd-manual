...
<main class="pt-24 pb-32 max-w-[1200px] mx-auto px-6 md:px-16 min-h-screen bg-white">
    ...
    <div class="grid grid-cols-1 gap-x-10 gap-y-20 md:grid-cols-3">
        ...
        <div class="flex flex-col items-center relative z-10">
            <div class="w-full aspect-[4/5] mb-6 relative flex items-center justify-center bg-transparent overflow-hidden rounded-2xl hover:shadow-md transition-shadow duration-300 border border-gray-100">
                <div class="absolute inset-0 z-0" style="background-color: <?= $pbg ?>;"></div>
                
                <?php if($p->discount): ?>
                <span class="absolute top-4 left-4 bg-black text-white text-[10px] font-bold px-3 py-1.5 uppercase tracking-widest shadow-md z-30 rounded-sm" style="font-family: 'Plus Jakarta Sans', sans-serif;"><?= esc($p->discount) ?></span>
                <?php endif; ?>
                
                <a href="<?= base_url('products/detail/' . urlencode($p->nama_produk)) ?>" class="absolute inset-0 z-20 flex items-center justify-center p-8">
                    <img alt="<?= esc($p->nama_produk) ?>" class="w-full h-full object-contain drop-shadow-md" src="<?= base_url('images/' . esc($p->gambar_produk)) ?>"/>
                </a>
            </div>
            
            <a href="<?= base_url('products/detail/' . urlencode($p->nama_produk)) ?>" class="block relative z-20 w-full">
                <h3 class="text-center font-bold mb-2 leading-snug text-lg text-black hover:text-gray-500 transition-colors line-clamp-2 px-4" title="<?= esc($p->nama_produk) ?>" style="font-family: 'Merriweather', Georgia, serif;">
                    <?= esc($p->nama_produk) ?>
                </h3>
            </a>
            ...
            <?php if(session()->get('isLoggedIn')): ?>
                <form action="<?= base_url('cart/add') ?>" method="POST" class="w-full text-center relative z-20">
                    ...
                    <button type="submit" class="border-2 border-black bg-black text-white px-8 py-3 text-xs font-bold transition-all w-[85%] mx-auto rounded-full uppercase tracking-widest hover:bg-white hover:text-black shadow-sm group-hover:shadow-md" style="font-family: 'Plus Jakarta Sans', sans-serif;">
                        Add to cart
                    </button>
                </form>
            <?php else: ?>
                <button class="trigger-login border-2 border-black bg-black text-white px-8 py-3 text-xs font-bold transition-all w-[85%] mx-auto rounded-full uppercase tracking-widest hover:bg-white hover:text-black shadow-sm group-hover:shadow-md relative z-20" style="font-family: 'Plus Jakarta Sans', sans-serif;">
                    Add to cart
                </button>
            <?php endif; ?>
